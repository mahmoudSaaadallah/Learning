...
-   **ASP.NET Core Web API Basics**: Understanding how controllers work, routing, and how HTTP requests are handled.
-   **Dependency Injection (DI)**: ASP.NET Core heavily relies on DI to provide services (like our database context) to controllers and other components.
...
-   **Data Integrity and Validation**: How is data validated before it hits the database? Is it just basic data annotations, or is there more robust business logic validation (e.g., using FluentValidation)?
...
-   **Service Layer**: For more complex applications, move business logic out of controllers into a separate service layer. Controllers then become thin, primarily handling HTTP requests and responses, delegating business logic to services.
    ```csharp
...
Your task is to extend our `CrudApi` project.

1.  **Create a new model**: Add a `Category` model (e.g., `Id`, `Name`, `Description`).
2.  **Establish a relationship**: Modify the `Product` model to include a `CategoryId` and a navigation property `Category` (one-to-many relationship: one category can have many products).
3.  **Update `ApplicationDbContext`**: Add a `DbSet<Category>`.
4.  **Add a new migration**: Generate and apply a new migration to update your database schema.
5.  **Create a `CategoriesController`**: Implement full CRUD operations for `Category` entities, similar to how we did for `Products`.
6.  **Enhance `GetProducts`**: Modify the `GetProducts` action in `ProductsController` to include the `Category` information when retrieving products (using `.Include()`).
7.  **Implement DTOs**: Create `CreateProductDto` and `ProductDto` (for output) for the `Product` entity. Update your `ProductsController` to use these DTOs for `POST` and `GET` operations.

This exercise will solidify your understanding of CRUD, relationships, and the importance of DTOs in a real-world scenario. Good luck!